...
import { isFailure, unwrap, successVoid, failure } from '../system/errors'
...
export default async function({
  ...
  if (buildTools) {
    try {
      ...
      return failure(`Error building mac tooling: ${String(error)}`)
    }
  }
  ...
  try {
    ...
    if (!String(error).includes('exists')) {
      ...
      return failure(`Issue creating symlink for xsbug.app: ${String(error)}`)
    } else {
